...
public class SyncResponse implements Serializable, SyncResponseInt {
    ...
    public SyncResponse(Configuration settings, List<Application> applications, Device device) {
        if (settings != null) {
            ...
            this.kioskMode = settings.isKioskMode();
            this.lockDefaultLauncher = settings.isLockDefaultLauncher();

            this.kioskExit = settings.getKioskExit();
            this.kioskHome = settings.getKioskHome();
            this.kioskRecents = settings.getKioskRecents();
            this.kioskNotifications = settings.getKioskNotifications();
            this.kioskSystemInfo = settings.getKioskSystemInfo();
            this.kioskKeyguard = settings.getKioskKeyguard();
            this.kioskLockButtons = settings.getKioskLockButtons();
            this.kioskScreenOn = settings.getKioskScreenOn();

            this.permissive = settings.getPermissive();
            this.lockSafeSettings = settings.getLockSafeSettings();
            this.showWifi = settings.getShowWifi();
            this.restrictions = settings.getRestrictions();
        }
