...
public class DeviceResource {
    ...
    // =================================================================================================================
    @ApiOperation(
            value = "Force device synchronization",
            notes = "Sends a configuration update notification to the selected device",
            response = Void.class
    )
    @POST
    @Path("/{number}/force-sync")
    @Produces(MediaType.APPLICATION_JSON)
    public Response forceSync(@PathParam("number") @ApiParam("Device number") String number) {
        try {
            ...
            log.error("Failed to force sync device: " + number, e);
